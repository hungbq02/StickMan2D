...
public sealed class ObjectPool : MonoBehaviour
{
    ...
    public static void CreatePool(string path, int initialPoolSize)
    {
        CreatePool(Resources.Load(path) as GameObject, initialPoolSize);
    }
...
// 			trans.localRotation = rotation;
    ...
    {
        return Spawn(Resources.Load(path) as GameObject, parent, position, rotation);
    }

    public static GameObject Spawn(string path, Transform parent, Vector3 position)
    {
        return Spawn(Resources.Load(path) as GameObject, parent, position, Quaternion.identity);
    }
    public static GameObject Spawn(string path, Vector3 position, Quaternion rotation)
    {
        return Spawn(Resources.Load(path) as GameObject, null, position, rotation);
    }
    public static GameObject Spawn(string path, Transform parent)
    {
        return Spawn(Resources.Load(path) as GameObject, parent, Vector3.zero, Quaternion.identity);
    }
    public static GameObject Spawn(string path, Vector3 position)
    {
        return Spawn(Resources.Load(path) as GameObject, null, position, Quaternion.identity);
    }

    public static GameObject Spawn(string path)
    {
        return Spawn(Resources.Load(path) as GameObject, null, Vector3.zero, Quaternion.identity);
    }

    public static IEnumerator Recycle(GameObject obj, float delay)
    {
	...
	public static void Recycle(GameObject obj, Transform parent)
	{
		GameObject prefab;
        if (instance.spawnedObjects.TryGetValue(obj, out prefab))
            Recycle(obj, prefab, parent);
        else
        {
	...
	static void Recycle(GameObject obj, GameObject prefab, Transform parent)
	{
		instance.pooledObjects[prefab].Add(obj);
		instance.spawnedObjects.Remove(obj);
        //if (obj.layer == instance.gameObject.layer)
        obj.transform.SetParent(parent);//instance.transform;
		obj.SetActive(false);
	}
    ...
    {
        instance.spawnedObjects.Remove(obj);
        instance.pooledObjects.Remove(obj);
        Object.Destroy(obj);
    }
